...
UltraFace::UltraFace(const std::string model_path,
	...
	ultraface = cv::dnn::readNetFromONNX(model_path); //(model_path + "/version-slim-320_without_postprocessing.onnx")
	ultraface.setPreferableBackend(cv::dnn::DNN_BACKEND_CUDA);// CUDA , OPENCV
    	ultraface.setPreferableTarget(cv::dnn::DNN_TARGET_CUDA);//CUDA_FP16
